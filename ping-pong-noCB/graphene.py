    ...
    for blk_size in BLK_SIZE:
        ...
        for bound in B:
            for fraction in FRACTION:
                ...
                print('Running %d trials for parameter combination: extra txns in mempool %d blk size %d fraction %f' % (NUM_TRIAL, true_false_positives, blk_size, fraction))
...
def main():
    fd =open(PATH+'/results/ping-pong-noCB-%d.csv' % (random.getrandbits(25)),'w')
    trial(fd)
    fd.close()
